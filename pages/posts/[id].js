...
export default function Post({ metadata, content, id }) {
    const title = regexifyString({
        pattern: /<.*?>/gim,
        decorator: (match, index) => <TextGradient key={index}>{match.slice(1, -1)}</TextGradient>,
        ...
        <Root title={metadata.title}>
            <Section>
                ...
                <div className="mt-4">
                    ...
                    •{" "}
                    <a target="_blank" rel="noreferrer" href={`https://github.com/nathan-pham/ffa-website-minimal/blob/main/pages/posts/markdown/${id}.md`} className="hover:underline cursor-pointer">
                        Edit
                    </a>
                </div>
